        ...
        {mode === 'login' && (
          <form onSubmit={e => { e.preventDefault(); handleLogin() }}>
            <div className="form-group">
              ...
              <input id="login-email" type="email" autoComplete="email" className="form-input" placeholder="example@seminary.ac.kr" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} />
            </div>
            <div className="form-group">
              <label htmlFor="login-pw">비밀번호</label>
              <input id="login-pw" type="password" autoComplete="current-password" className="form-input" placeholder="비밀번호를 입력하세요" value={loginPw} onChange={e => setLoginPw(e.target.value)} />
            </div>
            ...
            <div className="auth-switch">계정이 없으신가요? <button type="button" className="link-btn" onClick={() => { setMode('register'); setError('') }}>회원가입</button></div>
            <div className="auth-switch" style={{ marginTop: 8 }}><button type="button" className="link-btn" onClick={() => { setMode('reset'); setError(''); setResetStep(1) }}>비밀번호를 잊으셨나요?</button></div>
          </form>
        )}
        ...
        {mode === 'register' && (
          <form onSubmit={e => { e.preventDefault(); handleRegister() }}>
            ...
            <div className="form-group">
              ...
              <input id="reg-pw" type="password" autoComplete="new-password" className="form-input" placeholder="비밀번호를 입력하세요" value={regPw} onChange={e => setRegPw(e.target.value)} />
            ...
            <div className="form-group">
              ...
              <input id="reg-pw-confirm" type="password" autoComplete="new-password" className="form-input" placeholder="비밀번호를 다시 입력"
            ...
            <div className="form-group">
              ...
              <input id="reg-nickname" type="text" className="form-input" placeholder="커뮤니티에서 사용할 닉네임" maxLength={20}
                value={regNickname} onChange={e => setRegNickname(e.target.value)} />
            </div>
            <div className="form-group">
              <label htmlFor="reg-studentid">학생증 인증 <span style={{ color: 'var(--danger)' }}>*</span></label>
              <input
                id="reg-studentid"
                type="file"
                className="form-input"
            ...
            <div className="form-group">
              <label htmlFor="reg-note">인증 참고 메모 (선택)</label>
              <input id="reg-note" type="text" className="form-input" placeholder="예: OO신학대학원 재학 중 / 학번 등"
                value={regMembershipNote} onChange={e => setRegMembershipNote(e.target.value)} />
            </div>
            <button type="submit" className="auth-btn" disabled={!canRegister || busy}>{busy ? '신청 중...' : '가입 신청하기'}</button>
            <div className="auth-switch">이미 계정이 있으신가요? <button type="button" className="link-btn" onClick={() => { setMode('login'); setError('') }}>로그인</button></div>
          </form>
        )}
        ...
        {mode === 'verify-sent' && (
          <div>
            ...
            <div className="auth-switch" style={{ marginTop: 12 }}>
              <button type="button" className="link-btn" onClick={() => { setMode('login'); setError(''); setSentToEmail('') }}>로그인 화면으로</button>
            </div>
          </div>
        )}
            ...
            {resetStep === 1 && (
              <>
                <div className="form-group">
                  ...
                  <input id="reset-email" type="email" autoComplete="email" className="form-input" placeholder="example@seminary.ac.kr" value={resetEmail} onChange={e => setResetEmail(e.target.value)} />
                </div>
                <button type="button" className="auth-btn" onClick={verifyResetEmail} disabled={busy}>{busy ? '전송 중...' : '다음'}</button>
              </>
            )}
            {resetStep === 2 && (
              <>
                ...
                <div className="form-group">
                  ...
                  <input id="reset-pw" type="password" autoComplete="new-password" className="form-input" placeholder="새 비밀번호" value={resetPw} onChange={e => setResetPw(e.target.value)} />
                  <ul className="validation-list">
                    {pwRules.map(r => (
                      <li key={r.key} className={r.pass ? 'pass' : 'fail'}>
                        <span className="vicon">{r.pass ? '✓' : '✗'}</span> {r.label}
                ...
                <div className="form-group">
                  <label htmlFor="reset-pw-confirm">새 비밀번호 확인</label>
                  <input id="reset-pw-confirm" type="password" autoComplete="new-password" className="form-input" placeholder="비밀번호 확인" value={resetPwConfirm} onChange={e => setResetPwConfirm(e.target.value)} />
                  {pwMatch && <div className="email-check-msg ok">{'✓'} 비밀번호가 일치합니다.</div>}
                  {pwMismatch && <div className="email-check-msg err">비밀번호가 일치하지 않습니다.</div>}
                </div>
                <button type="button" className="auth-btn" onClick={handlePasswordReset} disabled={!canReset || busy}>{busy ? '변경 중...' : '비밀번호 변경'}</button>
              </>
            )}
            <div className="auth-switch">기억나셨나요? <button type="button" className="link-btn" onClick={() => { setMode('login'); setError('') }}>로그인</button></div>
          </form>
        )}
      </div>
    </div>
